...
object gbdt {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val table_in = "algo.xueyuan_onehot_new"//"algo.lookalike_feature_onehot"
    var imeiFile = ""
    //目标用户数
    var targetSize = 0
    //0不包含种子用户，1包含种子用户
    ...
    //predict
    var user_feature_forpre = otheruser_feature;
    if (inclusive == 1) {
      user_feature_forpre = user_feature.filter(r => (r._3.contains("," + uxip_boot_users_cycle + ","))).mapPartitions(iter => for (r <- iter) yield (r._1, r._2))
    }
  ...
  def load_data_onehot(table_in: String, uxip_boot_users_cycle: Int): RDD[(Long, SparseVector, String)] = {
...
//    c1.add(Calendar.DATE, -1)
    ...
    val sql_1 = "select imei,feature_size,feature,uxip_boot_users_cycle from " + table_in + " where stat_date=" + date1
